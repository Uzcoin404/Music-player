...
function musicPlayer() {
    ...
    for (let i = 0; i < musicList.length; i++) {
        let listLi = `  <li music-index="${i}">
                            <div class="list_music_left">
                                ...
                                <div class="music_list_img"><img src="${
                                    musicList[i].img
                                }" alt="" class="music_image"></div>
                            </div>
                            <div class="list_music_right">
                                <div class="list_music_info">
                                    <h4 class="list_music_name">${
                                        musicList[i].name
                                    }</h4>
                                    <p class="list_music_artist"><i class="fas fa-cloud-music"></i> ${
                                        musicList[i].artist
                                    }</p>
                                </div>
                                <audio class="music-${i}" src="${
            musicList[i].src
        }" hidden></audio>
    ...
    audio.addEventListener("timeupdate", function () {
        ...
        audioSpendTime.innerHTML = `${
            audioMinutes < 10 ? "0" + audioMinutes : audioMinutes
    ...
    audioRange.addEventListener("touchstart", function (e) {
        ...
        if (isDown) {
            ...
            audioCurrentTime =
                ((e.targetTouches[0].pageX - rect.left) / this.offsetWidth) *
                audio.duration;
            rangeSlider();
        }
    });
    ...
    document.addEventListener("click", function (e) {
        if (
            e.target == audioListIcon ||
            e.target == audioListIcon.querySelector("i")
        ) {
    ...
    volumeRange.addEventListener("mouseup", function () {
        isChange = false;
        controlIndicator.innerHTML = `Volume: ${Math.round(
            audio.volume * 100
        )}`;
        musicPlayer.classList.add("indi");
        musicLove();
    ...
    function colorChanger(color) {
        ...
        controlIndicator.innerHTML = `Color changed to ${root.style.getPropertyValue(
            "--color"
        )}`;
        musicPlayer.classList.add("indi");
        musicLove();
    ...
    document.addEventListener("click", function (e) {
        if (
            shortcutsClassChecker(e.target) ||
            e.target.className == "fas fa-info"
        ) {
